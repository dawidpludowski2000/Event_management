...
def test_promote_from_waitlist_happy_path():
    ...
    Reservation.objects.create(
        user=u2, event=event, status="pending", created_at=now - timedelta(minutes=30)
    )  # najstarszy
    Reservation.objects.create(
        user=u3, event=event, status="pending", created_at=now - timedelta(minutes=20)
    )
    Reservation.objects.create(
        user=u4, event=event, status="pending", created_at=now - timedelta(minutes=10)
    )  # najmłodszy
    ...
    with patch(
        "reservations.services.waitlist_service.send_reservation_status_email"
    ) as mail_mock:
    ...
    confirmed_after = Reservation.objects.filter(
    ...
    assert (
        Reservation.objects.filter(id__in=promoted_ids, status="confirmed").count()
        == expected_slots
    )
