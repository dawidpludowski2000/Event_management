...
urlpatterns = [
    ...
    path(
        "<int:event_id>/availability/",
        EventAvailablePlacesView.as_view(),
        name="event-available-seats",
    ),
    ...
    path(
        ...
        OrganizerMyEventsList.as_view(),
        name="organizer-events-list",
    ),
    # Organizator: edycja mojego wydarzenia
    path(
        "organizer/<int:event_id>/edit/",
        OrganizerEventUpdateView.as_view(),
    ...
    path(
        "organizer/metrics/", OrganizerMetricsView.as_view(), name="organizer-metrics"
    ),
]
